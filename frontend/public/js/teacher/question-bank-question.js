...
function page_url(sub_category) {
    $.ajax({
        method: 'GET',
        url: domain + '/api/admin/question-bank-by-name/' + sub_category,
        dataType: 'json',
...
function page_mcq(question_bank_sub) {
    $.ajax({
        method: 'GET',
        url: domain + '/api/admin/question-bank-mcq/' + question_bank_sub,
        dataType: 'json',
        success: function(data) {
            if (data.success == 1) {
                var html = ''
                for (let i = 0; i < data.data.length; i++) {
                    ...
                    html += '<span class="float-right text-danger delete-from-mcq-bank ml-2" action-id="' + data.data[i].question_bank_mcq_id + '" question_bank_sub="' + question_bank_sub + '" style="cursor: pointer;"><i class="fas fa-trash"></i></span>'
                    html += '<i class="float-right text-success mcq-q-edit" action-id="' + data.data[i].question_bank_mcq_id + '" style="cursor: pointer;"><i class="fas fa-edit"></i></i> '
...
$(document).on('click', '.add-question-option-submit', function() {
    ...
    $.ajax({
        method: 'POST',
        url: domain + '/api/admin/add-question-bank-mcq',
        dataType: 'json',
        data: {
            question_bank: category,
            question_bank_sub: sub_category,
...
$(document).on('click', '.delete-from-mcq-bank', function() {
    ...
    $.ajax({
        method: 'DELETE',
        url: domain + '/api/admin/question-bank-mcq-delete',
        dataType: 'json',
        data: {
...
$(document).on('click', '.mcq-q-edit', function() {
    ...
    $.ajax({
        method: 'GET',
        url: domain + '/api/admin/question-bank-mcq-by-question/' + $(this).attr('action-id'),
        dataType: 'json',
        success: function(data) {
            if (data.success == 1) {
